...
function updateYH_dialog() {
	...
	if (row.yhid) {
		$.ajax({
			...
			success : function(data) {
				var yhobj = eval("(" + data + ")");
				$('#updateyh_dlg').dialog('open').dialog('setTitle','查看/修改用户信息'); // 弹出窗口
				...
				$('#updateYH_YHZ_combobox').combobox('setValue',yhobj.json_yhzid);
			}
		});
	}
}
...
function showEnterDialog() {
	$.messager.confirm("删除信息", " 确认要删除选中的用户信息吗？", function(r) {
		if (r) {
			...
			$.ajax({
				url : "delYONGHU.action?nowtime=" + NowTime + "&yh_id="+ tmpyhid,
				context : document.body,
				success : function(data) {
					var yhobj = eval("(" + data + ")");
