...
function updateKECHENGB_dialog() {
    ...
    if (row.KCB_FXS_ID) {
        $.ajax({
            url: "updateKECHENGB.action?nowtime=" + NowTime + "&KCB_FXS_ID=" + row.KCB_FXS_ID, context: document.body,
            success: function (data) {
                var yhzobj = eval("(" + data + ")");
                $('#updateKECHENGB_dlg').dialog('open').dialog('setTitle', '查看/修改信息'); // 弹出窗口
...
function showEnterDialog() {
    $.messager.confirm("删除信息", " 确认要删除选中的信息吗？", function (r) {
        if (r) {
            ...
            $.ajax({
                url: "delKECHENGB.action?nowtime=" + NowTime + "&KCB_FXS_ID=" + tmpyhzid,
                context: document.body,
                success: function (data) {
                    var yhzobj = eval("(" + data + ")");
