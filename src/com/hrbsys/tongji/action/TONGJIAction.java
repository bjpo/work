...
public class TONGJIAction extends ActionBase {
	...
	public void listQUANXIAO1() throws Exception {
		...
		/**
		 * 分析： 1、统计全校信息分两种：实时和往天 2、实时查询为：不输入开始日期、结束日期
		 * 3、实时查询为：选择课程、选择教室、选择教工、选择课时 4、查询getKQQK() 5、展示到页面
		 * 6、往天查询：输入开始日期、结束日期（不输入则为开始日期） 7、调用kqxxservice查询结果。 8、展示到页面。
		 * **/
		dayin();
		// 如果开始日期不为空
	...
	public void listQUANXIAO2() throws Exception {
		/**
		 * 算法分析： 统计全校出勤情况时：分开按学院统计： 1、统计全校信息分两种：实时和往天 2、实时查询为：不输入开始日期、结束日期
		 * 3、实时查询为：选择课程、选择教室、选择教工、选择课时 4、查询getKQQK() 5、展示到页面
		 * 6、往天查询：输入开始日期、结束日期（不输入则为开始日期） 7、调用kqxxservice查询结果。 8、展示到页面。
		 * */
		int intPage = Integer.parseInt((page == null || page == "0") ? "1"
			...
			if (con.getZy_id() != null && !"".equals(con.getZy_id())) {
				...
				if (con.getNj_id() != null && !"".equals(con.getNj_id())) {
					...
					if (false) {
						...
						XUEQI xq=new XUEQI();
                                            if (this.KSRQ != null && this.JSRQ != null) {
