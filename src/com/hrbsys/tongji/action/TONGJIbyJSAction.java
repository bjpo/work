...
public class TONGJIbyJSAction extends ActionBase {
	...
	private String sort;
	//按照教室方式统计
	public void listTONGJIbyJS() throws Exception {
		...
		List<KECHENGB> all_kcb=kechengbService.findeKECHENGBByJSIDandXUEQI(js, xq,this.getKSRQ());
		if(all_kcb.size()<1){ //如果没有数据 ，则直接返回null
			jsonOutPutNULL();
			return ;
		}
		// 如果开始日期不为空
		if (((null != this.getKSRQ()) && (!"".equals(this.getKSRQ())))|| ((null != this.getZHOU())&& (!"".equals(this.getZHOU()))|| (null != this.getXINGQI() && !"".equals(this.getXINGQI())) || (null != this.getXUEQI_ID() && !"".equals(this.getXUEQI_ID())))) {
			...
			for(KECHENGB kb:all_kcb){
				HashMap<String, Integer> mapkqxxbj = kqxxbjxxService.getBJXXKQXXbyJS( this.getJS_ID(), this.getKS_ID(), this.getKCXX_ID(), xq, this.getXINGQI(), this.getZHOU(), this.getKSRQ(), this.getJSRQ());
				TONGJI tj = new TONGJI();
				if(null!=mapkqxxbj){
					tj.setKCXX_ID(kb.getKCB_FXS_ID());
					tj.setKCXX_KCMC(kb.getKCXXMC());
					tj.setJS_ID(js.getJS_ID());
					tj.setJS_JSMC(js.getJSMC());
					tj.setJS_ID(kb.getJS_ID());
					tj.setJS_JSMC(kb.getJSMC());
					
					tj.setJG_ID(kb.getLAOSHI_ID());
					tj.setJG_JGGH(kb.getLAOSHIGH());
					tj.setJG_JGMC(kb.getLAOSHIMC());
			...
			jsonMap.put("rows", listtj);
			jsonMap.put("page", intPage);
			new JsonPrintTools().printJSON(JSONObject.fromObject(jsonMap,config));
		}
	}
	// 按教室统计：详细信息
	public void listTONGJIbyJSdetial() throws Exception {
		int intPage = Integer.parseInt((page == null || page == "0") ? "1": page);
		int number = Integer.parseInt((rows == null || rows == "0") ? "10": rows);
		int start = (intPage - 1) * number;
		List<TONGJI> listtj = new ArrayList<TONGJI>();
		...
		if (((null != this.getKSRQ()) && (!"".equals(this.getKSRQ())))|| ((null != this.getZHOU())&& (!"".equals(this.getZHOU()))|| (null != this.getXINGQI() && !"".equals(this.getXINGQI())) || (null != this.getXUEQI_ID() && !"".equals(this.getXUEQI_ID())))) {
			...
			all_tongji.clear(); // 清空原有数据
			List<KQXX_XSCQ> allxs=kcxxxscqService.findKQXXbyJGdetail(this.getJS_ID(),null,this.getKS_ID(),this.getKCXX_ID(), xq,this.getXINGQI(),this.getZHOU(),this.getKSRQ(),this.getJSRQ()); //获取相应的学生考勤信息
			for(KQXX_XSCQ k:allxs){
				TONGJI t=new TONGJI();
				t.setJS_ID(k.getJS_ID());
				t.setJS_JSMC(k.getJSMC());
				t.setJG_ID(k.getJS_ID());
				t.setJS_JSMC(k.getJSMC());
			...
			// 如果没有查询天，则为当天，当天考勤信息为实时调用。
				String tmpandParam = ""; 
				List<XueShengKQ> allxk = realTimeService.getKQQK_XS(js.getJS_ID(),null,this.getKS_ID(),kb.getKCB_ID(), tmpandParam, null,null,null);
				for(XueShengKQ x:allxk){
					TONGJI t=new TONGJI();
					t.setJS_ID(x.getJs_id());
					t.setJS_JSMC(x.getJsmc());
					t.setJG_ID(x.getLs_id());
					t.setJS_JSMC(x.getJsmc());
					t.setJG_JGMC(x.getLsmc());
					t.setJG_JGGH(x.getLsgh());
					t.setJG_ID(x.getLs_id());
